                ...
                container('helm') {
                    script {
                        ...
                        if (releaseStatus.contains("Error: release: not found")) {
                            // Install new release
                            sh "helm install ${APP_NAME} ./crud-app  --set image.repository=${DOCKER_USERNAME}/${APP_NAME} --set image.tag=${APP_VERSION} ---atomic --force"
                        } else {
                            // Upgrade existing release
                            sh "helm upgrade ${APP_NAME} ./crud-app  --set image.repository=${DOCKER_USERNAME}/${APP_NAME} --set image.tag=${APP_VERSION} --atomic --force "
                        }
                    }
                }
            }
